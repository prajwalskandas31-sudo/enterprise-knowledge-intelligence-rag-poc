...
class TestCortexAnalystClient(unittest.TestCase):
    ...
    def test_request_payload_and_headers_construction(self, mock_post):
        ...
        # Verify headers and payload sent to requests.post
        mock_post.assert_called_once()
        _, kwargs = mock_post.call_args
        self.assertEqual(kwargs["headers"]["Authorization"], "Bearer test_pat_token_12345")
        self.assertEqual(kwargs["headers"]["X-Snowflake-Authorization-Token-Type"], "PROGRAMMATIC_ACCESS_TOKEN")
        self.assertEqual(kwargs["json"]["semantic_view"], "FDP_CORTEX_POC.RAW_DATA.CUSTOMER_ANALYTICS")
